...
namespace Game {
    public class GVSubterrainUpdater {
        ...
        public List<TerrainUpdater.LightSource> m_lightSources = [];
            ...
            for (int i = 0; i < 16; i++) {
                ...
                for (int k = num; k < num3; k++) {
                    for (int l = num2; l < num4; l++) {
                        switch (k) {
                            case 0:
                                if ((l == 0 && chunkAtCoords == null)
                                    || (l == 15 && chunkAtCoords6 == null)) {
                                    continue;
                                }
                                break;
                            case 15:
                                if ((l == 0 && chunkAtCoords3 == null)
                                    || (l == 15 && chunkAtCoords8 == null)) {
                                    continue;
                                }
                                break;
                        }
        ...
        public void NotifyBlockBehaviors(TerrainChunk chunk) {
            foreach (SubsystemGVBlockBehavior blockBehavior in m_subsystemGVBlockBehaviors.BlockBehaviors) {
                blockBehavior.OnChunkInitialized(chunk, m_subterrainSystem);
            }
            ...
            for (int i = 0; i < 16; i++) {
                for (int j = 0; j < 16; j++) {
                    ...
                    while (num2 < 255) {
                        ...
                        if (contents != 0) {
                            List<SubsystemGVBlockBehavior> blockBehaviors = m_subsystemGVBlockBehaviors.GetBlockBehaviors(contents);
                            foreach (SubsystemGVBlockBehavior blockBehavior in blockBehaviors) {
